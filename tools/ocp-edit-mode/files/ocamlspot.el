...
(defvar ocamlspot-keymap
  (let ((map (make-sparse-keymap)))
            (define-key map "\C-c;" 'ocamlspot-query)
            (define-key map "\C-c:" 'ocamlspot-query-interface)
            (define-key map "\C-c'" 'ocamlspot-query-uses)
            (define-key map "\C-c\C-t" 'ocamlspot-type)
            (define-key map "\C-c\C-i" 'ocamlspot-xtype)
            (define-key map "\C-c\C-y" 'ocamlspot-type-and-copy)
            (define-key map "\C-cx" 'ocamlspot-expand)
            (define-key map "\C-c\C-u" 'ocamlspot-use)
            (define-key map "\C-ct" 'caml-types-show-type)
            (define-key map "\C-cp" 'ocamlspot-pop-jump-stack)
    map))

(define-minor-mode ocamlspot-mode
  "OCaml auto-completion, documentation and source browsing using ocamlspot"
  :group 'ocamlspot
  :keymap ocamlspot-keymap
  )

...
let safe_make_dir (dir : FileAbs.t) =
  safe_mkdir (FileAbs.to_string dir)
...
let cache_log b fmt =
  Printf.kprintf (fun s ->
    output_string b.build_cache_log s;
    if verbose 4 then
  ...
  fun b r ->
    if r.rule_forced then true else
      ...
      IntMap.iter (fun _ f ->
        ...
        if f.file_mtime = BuildMtime.zero then
          missing_target := true
      (*      else
      ...
      IntMap.iter (fun _ f ->
        if f.file_exists then begin
          Printf.bprintf cmdbuf "#source:%s %s\n" (file_filename f) (BuildMtime.to_string f.file_mtime);
      ...
      *)
      let rec iter cmdbuf commands =
        ...
        | cmd :: commands ->
         ...
         (* MoveIfExists() is only for non-targets ! Thus, it is
            not taken into account in the compilation results.

            TODO: we need to think more that. Maybe there are cases
            where we just want to rebuild to get compilation
            information ?

            Printf.bprintf cmdbuf "#move? %s %s\n"
            (FileAbs.to_string f1) (FileAbs.to_string f2);
      ...
      let command_need_execution =
        try
          ...
          in
          cache_log b "#CACHE was %s\n" (OcpDigest.to_hex old_digest);
          if command_need_execution then begin
            cache_log b"#COMMAND ALREADY NEEDS EXECUTION\n";
            command_need_execution
          end else
            if old_digest = command_digest then begin
              cache_log b "#NO NEED FOR EXECUTION CONFIRMED\n";
              command_need_execution
            end else begin
              cache_log b "#EXECUTION WOULD BE NEEDED\n";
              true
            end
        with Not_found -> (* command_need_execution *)
          cache_log b "#NO CACHE: must be executed\n";
          true
      in
      if command_need_execution then begin
...
let experimental =
  try
    ignore (Sys.getenv "OCPBUILD_SAFE_MODE" : string);
    false
  with Not_found -> true
      ...
      begin
        ...
        | FILE_REAL ->
          try
            ...
            f.file_exists <- true;
            f.file_mtime <- BuildMtime.compute filename;
            if verbose 7 then Printf.eprintf " exists with mtime %s"
...
let rule_executed b r execution_status =
  ...
  if FileAbs.exists temp_dir then begin
    BuildEngineReport.cmd_rmdir (FileAbs.to_string temp_dir);
...
(* TODO: Use FileAbs.copy Copy line-oriented file *)
...
let different_digests fa1 fa2 =
  (Digest.file fa1) <> (Digest.file fa2)
...
let command_executed b proc status =
  ...
  | Some cmd ->
    let r = proc.proc_rule in
    let (_, begin_time) = IntMap.find r.rule_id b.build_stats_running_rules in
    let t =
      MinUnix.gettimeofday() -. begin_time
    in
    b.build_stats_running_rules <- IntMap.remove r.rule_id
    ...
    (*      if status <> 0 then begin
            ...
            FileAbs.string_of_file (temp_stderr b r)
            ];
            end; *)
...
let parallel_loop b ncores =
  ...
  and execute_proc proc nslots =
    ...
    | cmd :: tail ->
      ...
      | NeedTempDir ->
        let temp_dir = BuildEngineRules.rule_temp_dir r in
        if not (FileAbs.exists temp_dir) then
   ...
   (*       let loader =
      ...
      | Copy (f1, f2) ->
        ...
        begin try
                FileAbs.copy_file ff1 ff2
          with e ->
            Printf.eprintf "Error copying %s to %s: %s\n%!" fa1 fa2
              (Printexc.to_string e);
            if not (FileAbs.exists ff1) then
              Printf.eprintf "\tSource file %s does not exist\n%!" fa1;
            if not (FileAbs.exists (FileAbs.dirname ff2)) then
              Printf.eprintf
      ...
      | MoveIfExists (f1, f2, link) ->
        ...
        if FileAbs.exists ff1 then
          begin try
                  ...
                  | None -> ()
                  | Some f3 ->
                    let src_file = FileAbs.to_string ff2 in
                    let dst_file =
                      FileAbs.to_string (BuildEngineRules.file_of_argument r f3) in
                    try
            ...
            with e ->
              ...
              if not (FileAbs.exists (FileAbs.dirname (ff2))) then
                Printf.eprintf "\tDestination directory %s does not exist\n%!"
                  (FileAbs.to_string (FileAbs.dirname (ff2)));
              exit2 ();
          end;
        execute_proc proc nslots
      | DynamicAction (msg, f) ->
...
let find_cycle b =
  ...
  IntMap.iter (fun _ r ->
     match !min with
     | None ->
        min := Some (r, r.rule_missing_sources)
     | Some (_, prev) ->
         if prev > r.rule_missing_sources then
           min := Some (r, r.rule_missing_sources)
  ) b.queue_waiting;
  match !min with
  | None -> false
  | Some (r, _) ->
  (* 2) iter from that rule until we find itself *)
    let queue = Queue.create () in
    Queue.add (r,[r, r.rule_main_target],IntMap.empty) queue;
    try
      while not (Queue.is_empty queue) do
        let (r, path, map) = Queue.take queue in
        let map = IntMap.add r.rule_id r map in
        IntMap.iter (fun _ f ->
          if not f.file_exists then begin
            List.iter (fun r2 ->
	      if IntMap.mem r2.rule_id b.queue_waiting then
		     ...
		     Printf.eprintf "     is needed by %s\n"
                   ...
                   let path = (r2, f) :: path in
                    Queue.add (r2, path, map) queue
                  end
            ) f.file_target_of
          end
        ) r.rule_sources;
...
let parallel_loop b ncores =
  ...
  if not (BuildEngineDisplay.has_error b
          || b.build_should_restart || raised_exn <> None) && waiting_len > 0
  then begin
    (match raised_exn with
    | None -> ()
    | Some exn ->
      Printf.eprintf "No error, but exception %S raised in loop\n%!"
        (Printexc.to_string exn)
    );

    if find_cycle b then BuildMisc.clean_exit 2;
...
let sanitize b delete_orphans is_ok =
  ...
  let dir = FileAbs.of_string b.build_dir_filename in
  ...
  let rec iter dir cdir =
    FileDir.iter (fun basename ->
      let filename = FileAbs.add_basename dir basename in
      if FileAbs.is_directory filename then
        try
        ...
        with Not_found ->
          ...
          | DeleteOrphanFilesAndDirectories ->
            decr orphan_files;
            (try FileAbs.remove filename with _ -> ())
    ) dir

...
*)
  ...
  "-define", Arg.String (fun s ->
    let (name, valeur) = OcpString.cut_at s '=' in
      match valeur with
        | "true" | "" ->
          BuildValue.set_global s (BuildValue.plist_of_bool true)
        | "false" ->
          BuildValue.set_global s (BuildValue.plist_of_bool false)
        | _ ->
          BuildValue.set_global s (BuildValue.plist_of_string valeur)
  ),

...
(* val load_project : FileAbs.t list -> project * int *)

(* returns the number of errors while reading the files *)

val find_root : FileAbs.t -> string list -> FileAbs.t

(*
  val save_project : FileAbs.t -> project -> unit
*)

val scan_root : FileAbs.t -> FileAbs.t list


(* [find_package pj file] returns the list of packages in
   project [pj] containing [file] as a source.
val find_package : project -> FileAbs.t -> final_package list
*)

val save_project_state : project -> FileAbs.t -> unit
val load_project_state : FileAbs.t -> project

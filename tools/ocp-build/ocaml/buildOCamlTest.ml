...
let verbose = OcpDebug.verbose_function ["B"; "BuildOCamlTest"]
...
let check_output options subst output  option_name output_name =
  let output_check = BuildValue.get_strings_with_default options
      option_name []
  in
...
let find_binaries b cwd lib =
  let has_asm =
    BuildValue.get_bool_with_default lib.lib_opk.opk_options "asm" true in
  let has_byte = BuildValue.get_bool_with_default
    lib.lib_opk.opk_options "byte" true in
  let get_binary ext =
    let binary_basename =
      lib.lib.lib_name ^ ext
    in
    let binary =
      FileGen.to_string (FileGen.add_basenames b.build_dir
      ...
      let binaries =
        if lib.lib_sources = [] then
          ...
          begin
            match !program with
              None ->
                let cmd = BuildValue.get_strings_with_default
                  lib.lib_opk.opk_options
                  "test_cmd" [] in
              if cmd = [] then begin
    ...
    List.iter (fun (kind, binary) ->
      ...
      List.iter (fun (test, options) ->
        ...
        let subst = StringMap.add "tests"
            (FileGen.to_string
               (FileGen.add_basename lib.lib.lib_src_dir.dir_file "tests"))
            subst
        in
        let subst = StringMap.add "sources"
            (FileGen.to_string lib.lib.lib_src_dir.dir_file) subst in

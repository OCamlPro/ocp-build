...
let create_process cmd maybe_chdir stdin stdout stderr =
  match cmd with
    [] -> `EXN (Invalid_argument "create_process: empty command")
  | cmd :: args ->
...
let get_stdout_lines cmd args =
  ...
  match create_process (cmd@args) None None (Some temp_file) None with
  | `PID pid ->
...
let exists_as_directory filename =
  ...
  with _ ->
    ...
    if len > 0 && (let c = filename.[len-1] in c = '\\' || c = '/') then
      try Sys.is_directory (filename ^ ".") with _ -> false
    else false
